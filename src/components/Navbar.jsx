    ...
    <>
      <div className="flex justify-around  items-center gap-5   shadow-md">
        <div >
         ...
         {!isSidebar &&  <a className='lg:hidden block'><HomeIcon /></a> }
        </div>
        ...
        <div onClick={() => setIsSidebar(prev => !prev)} className={`cursor-pointer relative lg:hidden  transition-all duration-300 ease-in-out  ${isSidebar ? 'right-45' : 'right-5'}   md:${isSidebar ? 'right-20' : 'right-0'
      ...
      {
        <div className={`fixed top-0 z-[99999] right-0 lg:hidden  h-full w-64 px-4 transition-all duration-300 ease-in-out ${isSidebar ? 'right-0' : 'right-[-300px]'
          }`}   >
